...
const generateService = (callback) => {
  const taskReceivedCallback = (task, completionCallback) => {
    if (task.taskType === 'dataLink') {
      return dataLink.process(task, moduleGenerator.generate(task), completionCallback);
    } else if (task.taskType === 'businessLogic') {
      return businessLogic.process(task, moduleGenerator.generate(task), completionCallback);
    }
  };
