...
function _log(level, message) {
  ...
  const logMessage = {
    message,
    level,
    dlcId: process.env.DLC_ID,
    backingServerId: process.env.BACKING_SERVER_ID
  };

  const logMessageString = `${util.format(JSON.stringify(logMessage))}\n`;

  process.stdout.write(logMessageString);
