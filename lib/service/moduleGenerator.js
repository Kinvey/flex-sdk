...
function generateModules(task) {
  ...
  task.requestMetadata.securityContext = getSecurityContextString(task.request.headers.authorization, appMetadata);
  ...
  return {
    ...
    dataStore: dataStore(appMetadata, task.requestMetadata, taskMetadata),
    email: email(appMetadata),
    endpointRunner: endpointRunner(appMetadata, task.requestMetadata, taskMetadata),
    groupStore: groupStore(appMetadata, task.requestMetadata, taskMetadata),
    kinveyEntity: entity(appMetadata._id, useBSONObjectId),
    kinveyDate,
    push: push(appMetadata),
    Query,
    requestContext: requestContext(task),
    roleStore: roleStore(appMetadata, task.requestMetadata, taskMetadata),
    tempObjectStore: tempObjectStore(task.request.tempObjectStore),
    userStore: userStore(appMetadata, task.requestMetadata, taskMetadata)
  };
}
