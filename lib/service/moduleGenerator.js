...
function generateModules(task) {
  ...
  const appMetadata = {
    ...
    baasUrl: task.baasUrl || task.appMetadata.baasUrl || `${task.request.headers['x-forwarded-proto'] || 'https'}://${task.request.headers.host}`
  };
  ...
  return {
    ...
    email: email(proxyURL, appMetadata, taskMetadata, requestMetadata),
    groupStore: groupStore(appMetadata, requestMetadata, taskMetadata),
    kinveyEntity: entity(appMetadata._id, useBSONObjectId),
    kinveyDate,
    push: push(proxyURL, appMetadata, taskMetadata, requestMetadata),
    Query,
