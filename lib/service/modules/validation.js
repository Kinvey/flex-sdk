...
const validate = (expected, validationType, propertyName, propertyValue) => {
  ...
  switch (validationType) {
    ...
    case 'pattern':
      error = [];
      console.log('in pat. propertyValue: ' + propertyValue + ', expected: ' + JSON.stringify(expected));
      for (let index in expected) {
        const pattern = expected[index];
        console.log('pattern: ' + pattern);
        const regex = new RegExp(pattern);
        console.log(util.inspect(regex));
        console.log('r.test: ' + regex.test(propertyValue));
...
module.exports = {
  doValidation: (spec, objectToValidate) => {
    ...
    for (let attrName in spec) {
      if (!_.has(spec, attrName)) continue;
      const attrSpec = spec[attrName];
      for (let validationType in attrSpec) {
        console.log('validating: ' + validationType + ' in ' + JSON.stringify(attrSpec));
        if (!_.has(attrSpec, validationType)) continue;
        const expectedValue = attrSpec[validationType];
        console.log('expectedValue: ' + JSON.stringify(expectedValue));
