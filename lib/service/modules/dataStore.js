...
function initDataStore(appMetadata, requestContext, taskMetadata) {
  class DataStore extends BaseStore {
    ...
    collection(collectionName) {
      const _throwEntityRequiredError = (argName, arg, callback) => {
        const cb = typeof arg === 'function' ? arg : callback;
        const err = new Error('DataStoreError');
        err.description = 'Bad Request';
        err.debug = `${argName} is required`;
        return cb ? setImmediate(() => cb(err)) : Promise.reject(err);
      };

      const create = (entity, callback) => {
        if (!entity || typeof entity === 'function') {
          return _throwEntityRequiredError('entity', entity, callback);
        }
      ...
      const save = (entity, callback) => {
        ...
        if (Array.isArray(entity)) {
          const err = new Error('DataStoreError');
          err.description = 'Unable to save an array of entities. To insert multiple entities use "create" function.';
      ...
      const findById = (entityId, callback) => {
        if (!entityId || typeof entityId === 'function') {
          return _throwEntityRequiredError('entityId', entityId, callback);
        }
