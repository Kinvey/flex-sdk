...
module.exports = class Query {
  ...
  mod(field, divisor, remainder = 0) {
    ...
    const modRemainder = typeof remainder === 'string' ? parseFloat(remainder) : divisor;
  ...
  process(dataToProcess) {
    ...
    if (data) {
      ...
      if (json.sort != null) {
        data.sort((a, b) => {
          Object.entries(json.sort).forEach(([field, value]) => {
            // Find field in objects.
            const aField = nested(a, field);
            const bField = nested(b, field);
            const modifier = value; // 1 (ascending) or -1 (descending).
