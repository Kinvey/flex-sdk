  ...
  function _generateKinveyEntity(entityOrId) {
    ...
    if (typeof entityOrId === 'string' || typeof entityOrId === 'undefined') {
      ...
      if (typeof entityOrId === 'object' && entityOrId !== null) {
        ...
        if (isEmpty(_entity._acl)) _entity._acl = {};
        if (isEmpty(!_entity._id)) _entity._id = _createEntityId();
        if (isEmpty(_entity._acl.creator)) _entity._acl.creator = environmentId;
        if (isEmpty(_entity._kmd)) _entity._kmd = {};
        if (isEmpty(_entity._kmd.ect)) _entity._kmd.ect = currentTimeString;
        if (isEmpty(_entity._kmd.lmt)) _entity._kmd.lmt = currentTimeString;
      }
    }
    ...
    function getCreator() {
      return _.get(_entity, ['_acl', 'creator']);
    }

    function getReaders() {
      return _.get(_entity, ['_acl', 'r'], []);
    }

    function getWriters() {
      return _.get(_entity, ['_acl', 'w'], []);
    }

    function getReaderGroups() {
      return _.get(_entity, ['_acl', 'groups', 'r'], []);
    }

    function getWriterGroups() {
      return _.get(_entity, ['_acl', 'groups', 'w'], []);
    }
    ...
    function removeReader(userId) {
      let userIx = (_.get(_entity, ['_acl', 'r'], [])).indexOf(userId);
      while (userIx !== -1) {
    ...
    function removeWriter(userId) {
      let userIx = (_.get(_entity, ['_acl', 'w'], [])).indexOf(userId);
      while (userIx !== -1) {
    ...
    function removeReaderGroup(groupId) {
      let groupIx = (_.get(_entity, ['_acl', 'groups', 'r'], [])).indexOf(groupId);
      while (groupIx !== -1) {
